...
class Volleyball {
    constructor(x, y) {
        ...
        this.velocityX = 0;
        this.velocityY = 0;
        this.gravity = 1200; // Stronger gravity for better feel
        this.bounce = 0.6;
        this.friction = 0.95;
    ...
    // Hit the volleyball from a character
    hitFromCharacter(character) {
        const now = Date.now();
        if (now - this.lastHitTime < 200) return false; // Prevent spam hitting
        
        this.lastHitTime = now;
        
        // Calculate direction from character to ball
        ...
        const deltaX = ballCenterX - charCenterX;
        const deltaY = ballCenterY - charCenterY;
        ...
        const hitForce = 400;
        this.velocityX = normalizedX * hitForce + (Math.random() - 0.5) * 100;
        this.velocityY = normalizedY * hitForce - 200; // Always give some upward force
